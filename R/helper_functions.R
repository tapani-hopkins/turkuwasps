...
#'
#' @param x Vector of which trap, site or other location each wasp came from. Either as strings or factor.
#'
...
get_defaults = function(x){
	...
	# ..stop if it is unclear what the locations are
		...
		stop("Could not figure out if the locations were traps or sites. Check that thye are either all traps or all sites, and that every one is valid.")
	}	
